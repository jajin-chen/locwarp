...
不在電腦旁也能用手機操控 LocWarp。底部狀態列「**手機操控**」按鈕會顯示完整的 LAN 操控網址,把該網址貼到手機瀏覽器即可直接開啟行動裝置版地圖與功能鈕,不需要再輸入 PIN 或進行配對:
...
- 完整網址內含隨機 32-hex 存取 token,等同手機操控權限,請勿分享;缺少或錯誤 token 會以 401 拒絕
- 「重新產生操控網址」可立刻換一組 token,讓舊網址失效
